...
io.on('connection', function (socket) {
  socket.on('online', function (id) { 
    socket.playername = id; //在socket缓存id
    ...
    if (test === -1) { //无人等待匹配
      ...
      if (lastRival[player1.playername] === player2.playername) { //与上场比赛是同一对手，所以不能匹配
        ...
        MatchState.findOne({
          ...
          doc.lastrival = player2.playername;
          doc.state = 1;
          doc.save();
        });
        MatchState.findOne({
          ...
          doc.lastrival = player1.playername;
          doc.state = 1;
          doc.save();
        });
  ...
  socket.on('disconnect', function (reason) {
    ...
    if (key != -1) { //未匹配用户断线
      ...
      delete cache[socket.playername]; //如果断线用户上传了成绩一定要删除，下次再重新上传
    }
  });
